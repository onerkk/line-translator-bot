...
import factory_sequence_semantics as sequence_semantics
from translation_request_cache import memoize
import unicodedata

BUILD_ID = "2026-09-08.2-measurement-sequence-report-question"
...
def build_relations(source):
    ...
    for item, text in scopes:
        ...
        for transfer in (record_semantics.build_transfers(text, intro)
                ...
                + sequence_semantics.build_relations(text)):
...
def validate_relations(relations, translation):
    ...
    for relation in relations:
        ...
        elif kind in {'measurement_before_production', 'report_location_question'}:
            good = sequence_semantics.validate_relation(relation, text)
