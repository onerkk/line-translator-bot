...
FACTORY_SEMANTIC_AUDIT_BUILD_ID = "2026-07-24.2-source-claim-structured-audit-fallback"
...
def build_source_frame(source: str, src_lang: str, tgt_lang: str) -> Dict[str, Any]:
    ...
    flags["bulk"] = _contains_any(compact, ("大量", "大批", "批量", "很多"))
    flags["concentrated"] = _contains_any(compact, ("集中", "密集", "同時", "同时", "一起到"))
    flags["priority"] = _contains_any(compact, ("優先", "优先", "先做", "先跑", "先從", "先从", "先行生產", "先行生产"))
    ...
    frame["active"] = bool(frame["claims"] and frame["risk_score"] >= 3)
    return frame
