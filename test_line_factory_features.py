...
def hub(storage, monkeypatch, tmp_path):
    ...
    host = {"factory_line_settings": {"groups": {}, "stations": []},
            ...
            "_factory_stop_sender": lambda *args: stop_sends.append(copy.deepcopy(args)),
            "factory_stop_sends": stop_sends,
...
def test_acknowledgements_are_atomic_user_actions_and_replay_cannot_undo(hub, monkeypatch):
    ...
    assert record["responses"][COLLEAGUE]["status"] == "understood"
    assert record["responses"][COLLEAGUE]["event_timestamp"] == 200
    hub.postback(event(uid=COLLEAGUE, stamp=500), {"action": "factory_help", "token": token})
    assert hub.store.get("notice:" + GROUP + ":" + token) == record
