...
CUSTOMER_NAMES = sorted(list(STORAGE_LOOKUP.keys()), key=lambda x: -len(x))
...
def translate_openai(text, src, tgt, strict_no_source_script=False, repair_mode=False, bad_result=None):
    ...
    try:
        ...
        # Fix known GPT translation mistakes (but don't restore customers yet - validation needs clean result)
        if src == "zh":
...
def translate(text, src, tgt):
    ...
    # Pre-process: extract customer placeholders for zh source
    cust_placeholders = {}
    if src == "zh":
        _, cust_placeholders = pre_replace_zh(text)
    ...
    if result and is_translation_valid(result, src, tgt):
        # Restore customer names AFTER validation
        if src == "zh" and cust_placeholders:
    ...
    if result:
        ...
        if repaired and is_translation_valid(repaired, src, tgt):
            if src == "zh" and cust_placeholders:
                repaired = restore_customers(repaired, cust_placeholders)
