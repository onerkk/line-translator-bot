...
def translate_openai(text, src, tgt, strict_no_source_script=False, repair_mode=False, bad_result=None):
    ...
    try:
        ...
        sys_prompt = (
            ...
            "台芝, 百堅, 津展, 曜麟, 廉錩, shinko, 盛昌遠, 永吉, wing keung, GLH, 光輝, 寶麗金屬 etc. are customer names - NEVER translate. "
            ...
            "爐號=heat number/nomor furnace (NEVER translate 爐 as 'panas'), 爐號標籤=label heat number, "
            "不同爐號合併=gabung heat number berbeda, "
            ...
            "之後有包到寶麗金屬注意一下，有一批訂單會備註客戶不要爐號標籤 → Nanti kalau ada packing untuk 寶麗金屬 perhatikan ya, ada beberapa order yang dicatat pelanggan tidak mau label heat number. "
            "爐號標籤=label heat number (BUKAN 'label nomor panas'), 不要爐號標籤=tidak mau label heat number. "
            "有包到X=kalau ada packing untuk X (bukan 'paket datang ke X'). "
            "【台灣口語→印尼文 (設備/管理)】"
