...
(async()=>{
 ...
 const viewed=await (await fetch(base+'/preview-receipt-reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'factory_receipts',uid:'U'+'a'.repeat(32),timestamp:590})})).json();
 assert.equal(viewed.emitted_count,1);assert(viewed.feedback.includes('Status jawaban'));assert(!viewed.feedback.includes('PMI'));
 ...
 d.querySelector('#fa-load-receipts').click();await until(()=>d.querySelector('#fa-receipts').textContent.includes('查閱按鈕紀錄（不算回覆）：管理者'),'owner status view in admin');
 const removedHelp=await (await fetch(base+'/preview-receipt-reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'factory_help',timestamp:600})})).json();
 assert.equal(removedHelp.emitted_count,0);assert.equal(removedHelp.feedback,'');
 const acknowledged=(await (await fetch(base+'/api/admin/factory/receipts?group_id='+firstGroup)).json()).notices;
 assert(acknowledged.some(row=>row.responses['U'+'b'.repeat(32)]?.status==='understood'));
 const legacy=await (await fetch(base+'/preview-legacy-help',{method:'POST'})).json();
 ...
 const originalNow=w.Date.now;w.Date.now=()=>originalNow()+21000;w.dispatchEvent(new w.Event('focus'));await until(()=>d.querySelector('[data-token="'+legacy.token+'"]')?.textContent.includes('已知成員未回覆：Adi'),'legacy help is pending on visible receipt auto refresh');
 assert(!/需要說明|需說明|Perlu penjelasan|請發起人協助說明/.test(d.querySelector('#fa-receipts').textContent));
 ...
 const selected=await (await fetch(base+'/preview-ack-reminder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({repeat:true,scope:'mentioned'})})).json();
 assert.equal(selected.state,'repeat_pending');
 assert.deepEqual(Object.values(selected.messages[0].substitution).map(item=>item.mentionee),[{type:'user',userId:'U'+'b'.repeat(32)}]);
 assert(JSON.stringify(selected.messages[1]).includes('追蹤：指定 1 人'));
 assert(!JSON.stringify(selected.messages[1]).includes('名單不完整'));
 d.querySelector('#fa-load-receipts').click();
 await until(()=>d.querySelector('[data-token="'+selected.token+'"]')?.textContent.includes('僅追蹤指令中 @ 的指定成員'),'selected audience in admin');
 const selectedCard=d.querySelector('[data-token="'+selected.token+'"]');
 assert(selectedCard.textContent.includes('指定成員未回覆：Adi'));
 assert(!selectedCard.textContent.includes('Budi'));assert(!selectedCard.textContent.includes('名單不完整'));
 const outsider=await (await fetch(base+'/preview-receipt-reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:selected.token,uid:'U'+'c'.repeat(32),timestamp:1000})})).json();
 assert.equal(outsider.emitted_count,0);
 const beforeSelected=(await (await fetch(base+'/api/admin/factory/receipts?group_id='+firstGroup)).json()).notices.find(row=>row.token===selected.token);
 assert.deepEqual(beforeSelected.responses,{});assert.deepEqual(beforeSelected.pending_ids,['U'+'b'.repeat(32)]);
 const selectedReply=await (await fetch(base+'/preview-receipt-reply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:selected.token,timestamp:1100})})).json();
 assert.equal(selectedReply.emitted_count,0);
 d.querySelector('#fa-load-receipts').click();
 await until(()=>d.querySelector('[data-token="'+selected.token+'"]')?.textContent.includes('指定成員未回覆為 0，已自動停止提醒'),'selected audience completion');
 assert(!d.querySelector('[data-stop-token="'+selected.token+'"]'));
 console.log('PASS audience: native selected mentions, matching card/admin scope, unrelated taps ignored, selected completion stops reminders');
