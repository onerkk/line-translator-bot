## 2026-09-08：共用原文上下文修正（最新更新）

以 GitHub commit `ad4c2f8dad333d1a31010b5e0d9f5cb6310c178e` 為基準，交付 **17 個新增或修改檔案**，全部一次上傳至專案根目錄，保留其他檔案。務必包含新增的 `conversation_context.py`。

修正「上下文已開，但『放了』沒有前文」：先保存收到的原文，跨程序、跨語言讀取；首次翻譯、語義驗證、快取判斷、待重試工作及風格按鈕共用原訊息當時的語境。`udah → sudah` 正規化也保留關聯。普通聊天語境不再固定增加一輪 AI 覆核；不同私訊使用者分開處理。

全套 **1,620 項測試及 438 項子測試通過**，新增 40 項；工廠素材檢查無錯誤、無警告。離線重現中，正確候選一次生成即送出，錯誤候選維持拒收與重試。正式模型準確率和 LINE 回覆秒數尚待部署後實測。

詳見 [CONVERSATION_CONTEXT_FIX_20260908.md](CONVERSATION_CONTEXT_FIX_20260908.md)：問題證據、官方依據、更新與手機驗收步驟、儲存限制。沿用原本「群組 → 上下文」開關，沒有新的必填設定。

---

## 2026-09-08：優先放行未回覆修正（歷史更新）

本包以 GitHub commit `723b96bca57b9e6e98682a2bcf068aad48365bc0` 為基準，只含 **12 個修改或新增檔案**，全部放在專案根目錄，一次上傳並保留其他檔案。

修正「麻煩優先放行」的正常印尼文 `prioritaskan` 被誤判漏譯而攔截；接受正常放行文法，同時維持資料對象、明示目的地、優先順序及否定／完成狀態。英文句號切句也已補齊，避免借用其他句子的優先要求。沒有新的必填環境設定。

全套 **1,580 項測試及 438 項子測試通過**，本次新增 31 項。正常候選通過實際翻譯與送出流程，一次替代 AI 呼叫即可送出；LINE 傳送失敗後保留譯文補送。這是離線程式驗證，尚未取得當次線上 webhook／AI 回應／LINE 傳送紀錄，也未測量正式環境秒數。

證據、限制及上傳步驟見 [RELEASE_PRIORITY_AVAILABILITY_FIX_20260908.md](RELEASE_PRIORITY_AVAILABILITY_FIX_20260908.md)。請連同語義模組和測試檔一起更新，不要只上傳 app.py。

---

## 2026-09-07：帳務、標記與翻譯速度修正（歷史更新）

本包以 GitHub commit `b10eb100ba8620c0a01e071766b5c37c88941c80` 為基準，只含 **13 個修改或新增檔案**，全部放在專案根目錄。一次上傳，保留其他檔案；務必包含新增的 `factory_record_semantics.py`，沒有新的必填環境設定。

補上帳務紀錄／實體材料的區分、移入／移出與允許／禁止的核對、裝箱星號標記的語義，以及 `komputer → 電腦` 的臺灣用語。點名支援大小寫 Latin 名字；不再為尚未確定的巡廠消息自行追加表示成功的勾號。

一般術語範例提供給第一次翻譯提示，不再因本次新增的電腦術語範例而強制多做一輪複核。三個實際回報候選通過完整翻譯管線的離線測試，均只用一次替代 AI 呼叫；數量、否定與狀態錯誤仍會被拒絕。既有獨立複核、總呼叫預算、快取及重試機制保留。

完整 **1,549 項測試及 438 項子測試通過**，新增 60 項回歸測試；工廠素材檢查無錯誤、無警告。修正證據、官方技術依據與上傳步驟見 [TRANSLATION_QUALITY_SPEED_FIX_20260907.md](TRANSLATION_QUALITY_SPEED_FIX_20260907.md)。上述結果為離線程式驗證，尚未測量本次部署的實際 AI／LINE／Render 延遲。

---

## 2026-09-07：點名後文字漏譯修正（歷史更新）

本包以 `9fffa966375933bb75818e3aefa21fa8f768fa51` 為基準，只包含 6 個修改或新增檔案，全部放在專案根目錄。一次上傳，保留其他檔案，務必包含新增的 `translation_mentions.py`；沒有新的必填設定。

修正最後品質檢查把「@小麥（研磨股班長） tolong bukakan」整段誤當成人名，導致中文譯文被擋下的問題。點名辨識與品質檢查共用同一套規則，並保留名字內原有的連續空白。

新增 37 項回歸測試，完整 **1,489 項測試及 438 項子測試通過**，工廠資產檢查通過。保留上次加速修改，沒有新增 AI 呼叫。離線重現證據、上傳與手機驗收步驟見 [MENTION_TRANSLATION_FIX_20260907.md](MENTION_TRANSLATION_FIX_20260907.md)。圖片當時未翻譯的原因仍需正式執行紀錄確認。

---

## 2026-09-07：翻譯等待時間優化（歷史更新）

本包以 `ca8707af4139f1555eb7edad100939fffc1a3f01` 為基準，只包含 15 個修改或新增檔案，全部放在專案根目錄。請一次上傳覆蓋，保留其他檔案，並確認新增的 `translation_request_cache.py` 一併上傳；沒有新的必填設定。

共用單次請求內相同的語義分析及本地驗證，縮短知識案例讀取，並移除正常佇列讀取時反覆執行的資料庫結構遷移。保留既有模型、提示、品質檢查、持久佇列與群組隔離。

三輪離線對照中，長公告未命中快取的本機處理中位數由 160.9 ms 降至 99.5 ms，約減少 38%；其餘情境與限制見 [TRANSLATION_LATENCY_20260907.md](TRANSLATION_LATENCY_20260907.md)。這不含實際 AI／LINE／Render 等待時間。完整驗證：1,452 項測試、438 項子測試通過，工廠素材驗證通過。

---

## 2026-09-07：翻譯語意範圍審查與修正（歷史更新）

本包以 commit `6b399d895a6974fd9ecc03f128c02301835f4b55` 為基準，只包含 18 個修改或新增檔案，全部放在專案根目錄。請一次上傳覆蓋，保留原有其他檔案；不要只更新 `app.py`。下方舊更新包的操作說明屬歷史紀錄。

修正否定、疑問句被固定捷徑改成肯定句，跨語境替換把正確用詞改錯，正常詞義被品質檢查誤攔截，以及「尚未修好」被誤判為已完成等問題。合格短句與快取保留；預設模型及翻譯輪數未調高。

新增 82 項回歸測試；舊版可重現的 21 項失敗在修正版通過。完整測試為 **1,437 項測試、438 項子測試通過**，工廠素材發布驗證亦通過。

修正證據、LINE 官方規格核對與手機測試步驟見 [TRANSLATION_AUDIT_20260907.md](TRANSLATION_AUDIT_20260907.md)。離線測試確認程式行為，不代表所有 AI 譯文永遠零錯誤，也不代替線上速度與費用量測。

---

## 2026-09-07：統一 LINE 底部快捷選單（歷史更新）

本次更新以 commit `e103c3d8a1bff7e8dfba4531ddb230557ee710e0` 為基準，只包含修改及新增的檔案。請保留 ZIP 內的路徑，一併覆蓋根目錄、`static`、`tests` 與工作流程檔案。下方其他更新包的說明屬歷史紀錄。

- 唯一設定入口：**後台 → 快捷鍵**。先選「全群組預設」或指定群組，再調整總開關、排序、按鈕名稱、文字／圖片情境，最後按「儲存此設定」。
- 各群組可獨立設定；「恢復使用全群組預設」只影響目前選取的群組。第一次載入會保留舊版中與選單有關的有效設定；自訂名稱、刪除與排序在重啟後仍會保留。
- 管理員須有「快捷鍵」權限才可修改。群組裡的成員共用該群組的底部選單；這不是 LINE 原生群主身分識別功能。
- 文字、圖片、分享、工具、了解／說明／確認與外連按鈕共用同一份選單，不再由其他頁面追加。外連頁仍管理連結內容；是否放到選單、名稱及順序由快捷鍵控制。
- 公告卡片保留可再次點選的確認按鈕，與底部選單共用相同的開關和名稱。設定頁與工廠工具頁的重複按鈕設定已移除。
- [LINE 每則訊息最多 13 顆快捷鍵](https://developers.line.biz/en/docs/messaging-api/using-quick-reply/)，更多按鈕以分頁呈現。分頁只讀既有翻譯內容，不呼叫翻譯模型。語言選擇、掃碼等使用者主動開啟的操作步驟仍維持各自需要的選項。
- 儲存後套用到**接下來的新翻譯**；已送到 LINE 的歷史訊息不會被這次後台設定改寫。

新增 `test_unified_quick_reply.py` 及 `tests/unified_menu_ui_smoke.cjs`，驗證群組隔離、權限、設定遷移、分頁、圖片對照、公告紀錄、錯誤儲存與真實訊息組裝；測試使用本地替代的 LINE／AI 傳輸，不向正式群組發送訊息。

---

## 2026-09-07：LINE 工廠工具整合（歷史更新）
