...
def test_ambiguous_number_style_returns_only_same_area_not_a_guessed_length(length_lines):
    text = PHOTO_5.replace("長度MIN：2500\n長度MAX：2550", length_lines)
    result = extract_work_order_info(text, STORAGE, {})
    assert result["length"] is None
    assert result["storage"] == {"status": "ok", "area": "EH79", "customer": "方鉦"}
...
def test_ambiguous_style_crosses_storage_boundary_remains_unresolved():
    text = PHOTO_5.replace("長度MIN：2500\n長度MAX：2550",
                           "長度MIN：3.200 mm\n長度MAX：3.201 mm")
    result = extract_work_order_info(text, STORAGE, {})
    assert result["length"] is None
